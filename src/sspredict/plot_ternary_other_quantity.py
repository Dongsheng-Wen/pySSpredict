...
def main():
    parser = argparse.ArgumentParser(description='visualize other properties together with solid solution strengths',formatter_class=RawTextHelpFormatter)
    ...
    parser.add_argument('-property',type=str,default='Delta_Eb', help='Visualize other properties in the ternary plot, you can put:\n'
     'V_ave\n'
     'b_ave\n'
     'E_ave \n'
     'mu_ave\n'
     ...
     'Delta_Eb\n'
     'Delta_sigma_ss\n'
     'default input is Delta_Eb\n'
     'for more information of these properties, see examples/output_description.pdf or literatures in the reference')
    args = parser.parse_args()
    property_n = args.property
    st_name = args.str
    if args.pd !=None:
    ...
    if len(element) == 3:
        XYlist = [];Xlist = [];Ylist = [];Zlist = [];XYZlist = []; Z_xlist =[]
        ...
        for row in data.index:
            ...
            Z_x = np.float32(data[property_n][row]);
    ...
    level = np.arange(lowerlim, upperlim+1, 10);
    level2 = np.linspace(lowerlim_x, upperlim_x, 11);
    ticks1 = ([item for item in level if item % 50 ==0])
    if (upperlim - lowerlim) >= 500:
        ticks1 = ([item for item in level if item % 100 ==0])
    ticks2 = ([item for item in level2])
    ...
    # plot 2, property specified by user 
    plt.subplot(1, 2, 2)
    if args.pd != None:
        columnname=list(cols.columns.values)
        lines = max(cols[columnname[2]])
    ...
    #plt.clim(lowerlim_x, upperlim_x);
    if args.s != None:
